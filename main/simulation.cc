...
int main(int argc, char* argv[]){
  if(argc<=5){
    std::cerr << "Input: " << argv[0] << " [Residual nucleus] [ldmodel] [parity&optmodall] [flag_jpi] [verbosity] [Random Seed (optional)] [exp/thr (for 40Ar)]" << std::endl;
    return 0;
  }
  ...
  if(argc >= 7) seed = atoi(argv[6]); // Seed parsing stays exactly where it was
  
  string sf_type = "exp"; // Default to 'exp' if not provided
  if(argc >= 8) sf_type = argv[7];    // sf_type is shifted to the very end

  string res_nuc = argv[1];
  bool is_ar40 = (res_nuc == "39Ar" || res_nuc == "39Cl");

  std::cout << "VERBOSE = " << verbose << std::endl;
  std::cout << "SEED = " << seed << std::endl;
  if(is_ar40) std::cout << "SF TYPE = " << sf_type << std::endl;
  // ---- FIXME --- // 
  ...
  if(Z+N==11) Zt=6,Nt=6;
  if(Z+N==15) Zt=8,Nt=8;
  if(Z+N==39) Zt=18,Nt=22;
  string output_subdir;
  string sf_filename;
  bool sf_is_grid=0;
  double S; 

 const char* nucdeex_root = getenv("NUCDEEX_ROOT");
  if(nucdeex_root==NULL){
    std::cerr << "NUCDEEX_ROOT is not set" << std::endl;
    return -1;
  }
  ...
  if(flag_jpi){
    ...
    }else if(Z==17 && N==22){//39Cl
    Zt=18; Nt=22;
    output_subdir = "40Ar";
    os.str("");
    if(sf_type == "exp") os << nucdeex_root << "/tables/sf/pke_40Ar_exp.root";
    else                 os << nucdeex_root << "/tables/sf/gsf_Ar40P.root";
    sf_filename = os.str();
    sf_is_grid = 1;
    S = NucDeEx::Utils::NucleusTable->GetNucleusPtr("40Ar")->S[2];
  }else if(Z==18 && N==21){//39Ar
    Zt=18; Nt=22;
    output_subdir = "40Ar";
    os.str("");
    if(sf_type == "exp") os << nucdeex_root << "/tables/sf/pke_48Ti_exp.root";
    else                 os << nucdeex_root << "/tables/sf/gsf_Ar40N.root";
    sf_filename = os.str();
    sf_is_grid = 1;
    S = NucDeEx::Utils::NucleusTable->GetNucleusPtr("40Ar")->S[1];
  }else{
