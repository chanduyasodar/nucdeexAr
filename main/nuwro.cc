...
int main(int argc, char* argv[]){
  ...
  if(prefix.find("QE_C")!=string::npos){
    ...
    nuc_name="16O";
  }else if(prefix.find("QE_Ar")!=string::npos){
    Zt=18;
    Nt=22;
    os << getenv("NUCDEEX_ROOT") << "/tables/sf/pke_40Ar_exp.root";
    nuc_name="40Ar";
    flag_O=1;
  }
  ...
  for(int j=0;j<tree->GetEntries();j++){
    tree->GetEntry(j);
//find outgoing nucleon and assign correctS
   int struck_pdg = event->in[1].pdg;
	if(struck_pdg==2112){
		S=NucDeEx::Utils::NucleusTable->GetNucleusPtr(nuc_name)->S[1];
}else if(struck_pdg==2212){
                S=NucDeEx::Utils::NucleusTable->GetNucleusPtr(nuc_name)->S[2];
}else{
	continue;
}
 cout << "Separation E = " << S << endl;
    ...
    // --- DO SIMULATION --- //
// --- Skip events where residual nucleus has more protons or neutrons
//     than the target (unphysical / triggers NucDeEx TALYS lookup bug) --- //
if(event->pr > Zt || event->nr > Nt){
  if(NucDeEx::Utils::fVerbose>0){
    std::cout << "SKIP EventID=" << j
               << ": residual (Z=" << event->pr << ", N=" << event->nr << ")"
               << " exceeds target (Zt=" << Zt << ", Nt=" << Nt << ")" << std::endl;
  }
  continue;
}


    NucDeExEventInfo result = deex->DoDeex(Zt,Nt,event->pr,event->nr,Ex,TVector3(0,0,0));
