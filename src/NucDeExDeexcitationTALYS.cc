...
std::cout << "ROOT ID = " << fRootID
          << " target = " << name_target
          << std::endl;    
fNucleusID = getNucleusID(fRootID,name_target.c_str());
std::cout<<"Nucleus ID"<<fNucleusID<<std::endl;
...
if(i==7){std::cout<<"file7target"<<name_target<<std::endl;}
...
if(name_target=="39Ar"){
    std::cout << "REGISTERED 39Ar in file "
              << i
              << " nucleus_id="
              << nucleus_id_r
              << std::endl;
...
/////////////////////////////////////////////
{
 // First look for an actual ROOT-file nucleus
  it = map_root.find(name);
  if(it != map_root.end()){
    return (int) it->second;
  }

  // If not found, search the nuclei contained in the ROOT files
