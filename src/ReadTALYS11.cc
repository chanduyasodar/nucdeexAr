...
static bool EnsureNucleusData(NucDeExNucleus* nuc, const std::string& context)
{
  if(nuc==NULL){
    std::cerr << "ERROR: Unknown nucleus while reading TALYS " << context << std::endl;
    return false;
  }
  if(!nuc->HasDataArrays()) nuc->InitData();
  return true;
}
...
///////////////////////////
{
  ...
  while(_ifs->getline(buf,sizeof(buf))){
    ...
    if(find_discrete != std::string::npos){
      ...
      if(!EnsureNucleusData(nuc,"discrete-level block")) return 0;
      if(NucDeEx::Utils::fVerbose>0){
      ...
      if(find_discrete_br == std::string::npos){ // level info
        ...
        if(std::istringstream(st) >> d_bin >> d_energy >> d_spin >> d_parity){
          if(d_bin<0 || d_bin>=NucDeEx::bins){
            std::cerr << "ERROR: Discrete level bin out of range: " << d_bin
        ...
        if(discrete_bin<0){
          std::cerr << "ERROR: Found discrete level branching before a level line: "
              << buf << std::endl;
          return 0;
        }
        ...
        if(d_bin_daughter<0 || d_bin_daughter>=NucDeEx::bins){
          std::cerr << "ERROR: Discrete daughter level bin out of range: "
              << d_bin_daughter << " (compiled NucDeEx::bins="
              << NucDeEx::bins << ")" << std::endl;
          return 0;
        }
        nuc->level_br[discrete_bin][d_bin_daughter] = tmp_br;
        if(NucDeEx::Utils::fVerbose>0){
          std::cout << discrete_bin << " ---> " << d_bin_daughter  << " : " << nuc->level_br[discrete_bin][d_bin_daughter] << std::endl;
        }
      }
    }
    ...
    if(find_population != std::string::npos){
      ...
      nuc = _nucleus_table->GetNucleusPtr(z, n);
      if(!EnsureNucleusData(nuc,"population block")) return 0;
      ...
      if(find_decay == std::string::npos && flag_mode==0){
        ...
        if(!EnsureNucleusData(nuc,"decay block")) return 0;
        //if(NucDeEx::Utils::fVerbose>0){
