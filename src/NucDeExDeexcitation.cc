...
/////////////////////////////////////////////
{
  const bool is_supported_target =
    (Zt==6 && Nt==6) || (Zt==8 && Nt==8) || (Zt==18 && Nt==22);
  const bool has_phole_model =
    (Zt==6 && Nt==6) || (Zt==8 && Nt==8);
...
//pure talys model for 40Ar
  ...
  if( (Zt==Z && Nt==N+1) || (Zt==Z+1 && Nt==N) ){
   ...
   //12C and 16O
    ...
    if(Ex>min_S || !has_phole_model) EventInfo = deex_talys->DoDeex(Zt,Nt,Z,N,Ex,mom);
...
/////////////////////////////////////////////
{
  if(! ((Zt==6 && Nt==6 )||(Zt==8 && Nt==8)||(Zt==18 && Nt==22)) ){
    std::cerr << "ERROR: This tool does not support the target nucleus" << std::endl;
    exit(1);
  }
  if(NucDeEx::Utils::fVerbose>0){
