...
int plot_sf(){
  ...
  os.str("");
  if(flag==2){
    prefix = "pke_40Ar_exp"; // or "gsf_Ar40N" for neutron
  }else{
    os << "pke" << A;
    if(flag==0) os << "_tot";
    prefix = os.str();
  ...
  TH2D* h_sf_int = new TH2D("h_sf_int","",bin_p_int,min_p,max_p,bin_E_int,min_E,max_E); 
	double eps = 1e-3;
	double xlo = h_sf->GetXaxis()->GetBinCenter(1) + eps;
	double xhi = h_sf->GetXaxis()->GetBinCenter(bin_p-1) - eps;
	double ylo = h_sf->GetYaxis()->GetBinCenter(1) + eps;
	double yhi = h_sf->GetYaxis()->GetBinCenter(bin_E-1) - eps;
	for(int i=1;i<bin_p_int;i++){
		for(int j=1;j<bin_E_int;j++){
			...
			double Yc = std::min(std::max(Y,ylo),yhi);
			h_sf_int->SetBinContent(i,j,h_sf->Interpolate(Xc,Yc));
		}
	}
